...
class PRS():
    def __init__(self,M,err,x=3,chisq=4,norm=False,res=241):
        ...
        ----------
        M: 2d array
            M from PRSgamma output MAT file.
        err: 2d array
            err from PRSgamma output MAT file.
        x: float
            x from PRSgamma output MAT file. The default is 3.
        chisq: float, optional
            chi squared for PRS surface. The default is 4.
        norm: Boolean, optional
            Normalized PRSs if True, real PRSs if False. The default is False.
        ...
        #to save time, calculate mu exactly once per PRS object
    ...
    def Plot(self,ax,color,dash=False,area=False):
        """
        Plot the perimeter of a mu-lambda PRS cross-section and return gamma fit parameters for the cross-section.
        ...
        if dash:
            ax.plot(self.mu,la_l,color=color,lw=2,ls='--')
            ax.plot(self.mu,la_u,color=color,lw=2,ls='--')
        else:
            ax.plot(self.mu,la_l,color=color,lw=2)
            ax.plot(self.mu,la_u,color=color,lw=2)
        if area:
    ...
    def PRS(self,jumpup=True):
        """
        Return gamma fit parameters for a projection of the surface of a 3D PRS onto the mu-lambda axes.
        ...
        Returns:
            ...
            la_ML: float
                lambda for most likely gamma fit parameters.
            logN0_surf: 2d array
                log10(N0) for surface of 3D PRS.
            mu_surf: 2d array
                mu for surface of 3D PRS.
            la_surf: 2d array
                lambda for surface of 3D PRS.
        """
        psi = np.linspace(0,np.pi,self.res)
        yp = -np.log(gammaln(1+2*self.x+self.mu)+gammaln(1+self.mu)-2*gammaln(1+self.x+self.mu))
        sig2l = np.sqrt(self.chisq)*np.sin(np.transpose(psi))
    ...
    def Vertex(self):
        """
        Get gamma fit parameters for figures that use PRS vertices.
